...
public final class HologramRenderer implements AutoCloseable {
    ...
    private final int[] columnTextures={glGenTextures(),glGenTextures()};
    private boolean columnsUploaded;
    ...
    public void render(Game game,Camera camera,int depthTexture,int width,int height,float time,boolean paused) {
        ...
        shader.vector("panelSize",new Vec3(7.5f,1.875f,0));shader.integer("plainLabel",0);
        ...
        if(!columnsUploaded) { uploadColumns(false);uploadColumns(true);columnsUploaded=true; }
        boolean back=camera.position().z<0;float side=back?-1:1;
        shader.integer("plainLabel",1);shader.vector("anchor",new Vec3(0,6.03f,side*.34f));
        shader.vector("panelRight",new Vec3(side,0,0));shader.vector("panelSize",new Vec3(Board.COLUMNS,.40f,0));
        glBindTexture(GL_TEXTURE_2D,columnTextures[back?1:0]);glDrawArrays(GL_TRIANGLES,0,6);
    ...
    private void upload(HUD.Status status) {
        ...
        g.dispose();uploadImage(image,texture);
    }
    private void uploadColumns(boolean back) {
        int cell=320;
        BufferedImage image=new BufferedImage(cell*Board.COLUMNS,128,BufferedImage.TYPE_INT_ARGB);
        Graphics2D g=image.createGraphics();
        g.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON);
        g.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING,RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
        g.setFont(new Font(Font.SANS_SERIF,Font.BOLD,100));
        for(int column=0;column<Board.COLUMNS;column++) {
            // Reverse the atlas on the rear face so each physical shaft keeps its key.
            String number=Integer.toString(back?Board.COLUMNS-column:column+1);
            g.setColor(new Color(8,17,27,235));g.fillRoundRect(column*cell+(cell-96)/2,3,96,122,20,20);
            g.setColor(new Color(255,222,148));
            g.drawString(number,column*cell+(cell-g.getFontMetrics().stringWidth(number))/2,100);
        }
        g.dispose();uploadImage(image,columnTextures[back?1:0]);
    }
    private static void uploadImage(BufferedImage image,int texture) {
        int width=image.getWidth(),height=image.getHeight();ByteBuffer data=MemoryUtil.memAlloc(width*height*4);
        try {
    ...
    @Override public void close() { glDeleteTextures(texture);for(int id:columnTextures) glDeleteTextures(id);glDeleteVertexArrays(vao);shader.close(); }
}
